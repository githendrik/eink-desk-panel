...
void EPD_InitUTF8Fonts(uint16_t width, uint16_t height);  // Initialize UTF-8 font support
...
void EPD_ShowStringUTF8(uint16_t x, uint16_t y, const char *chr, uint16_t size1, uint16_t color);               //显示UTF-8字符串(支持德语字符)
int EPD_GetUTF8TextWidth(const char *chr, uint16_t size1);                                                      //获取UTF-8字符串宽度
