...
class EPD_GFX : public Adafruit_GFX {
  public:
    EPD_GFX(int16_t w, int16_t h) : Adafruit_GFX(w, h) {}
    
    void drawPixel(int16_t x, int16_t y, uint16_t color) {
      if (x < 0 || x >= _width || y < 0 || y >= _height) return;
      Paint_SetPixel(x, y, color == 0 ? WHITE : BLACK);
    }
};

// Global GFX and U8g2 font objects
EPD_GFX *epd_gfx = nullptr;
U8G2_FOR_ADAFRUIT_GFX *u8g2_fonts = nullptr;

// Initialize GFX and U8g2 fonts (call this once)
void EPD_InitUTF8Fonts(uint16_t width, uint16_t height) {
  if (epd_gfx == nullptr) {
    epd_gfx = new EPD_GFX(width, height);
  }
  if (u8g2_fonts == nullptr) {
    u8g2_fonts = new U8G2_FOR_ADAFRUIT_GFX();
    u8g2_fonts->begin(*epd_gfx);
  }
}
...
/*******************************************************************
    函数说明：显示UTF-8字符串(支持德语umlauts等扩展字符)
    接口说明：x     字符串x坐标参数
              y      字符串Y坐标参数
               chr    要显示的UTF-8字符串
              size1  显示字符串字号大小 (8, 12, 16, 24)
              Color  像素点颜色参数
    返回值：  无
*******************************************************************/
void EPD_ShowStringUTF8(uint16_t x, uint16_t y, const char *chr, uint16_t size1, uint16_t color)
{
  // Initialize if not already done
  if (u8g2_fonts == nullptr) {
    EPD_InitUTF8Fonts(Paint.Width, Paint.Height);
  }
  
  // Set font based on size - using pixel/sans-serif fonts with Latin-1 support
  if (size1 <= 8) {
    u8g2_fonts->setFont(u8g2_font_6x10_tf);  // 6x10 pixel font with Latin-1
  } else if (size1 <= 12) {
    u8g2_fonts->setFont(u8g2_font_7x13_tf);  // 7x13 pixel font with Latin-1
  } else if (size1 <= 16) {
  ...
  u8g2_fonts->setForegroundColor(color == BLACK ? 1 : 0);
  u8g2_fonts->setBackgroundColor(color == BLACK ? 0 : 1);
  
  // Draw the string - U8g2 will handle UTF-8 decoding
  u8g2_fonts->setCursor(x, y + size1);
  u8g2_fonts->print(chr);
}

/*******************************************************************
    函数说明：获取UTF-8字符串宽度
    接口说明：chr    要测量的UTF-8字符串
              size1  字号大小
    返回值：  字符串宽度(像素)
*******************************************************************/
int EPD_GetUTF8TextWidth(const char *chr, uint16_t size1)
{
  // Initialize if not already done
  if (u8g2_fonts == nullptr) {
    EPD_InitUTF8Fonts(Paint.Width, Paint.Height);
  ...
  if (size1 <= 8) {
    u8g2_fonts->setFont(u8g2_font_6x10_tf);
  } else if (size1 <= 12) {
    u8g2_fonts->setFont(u8g2_font_7x13_tf);
  } else if (size1 <= 16) {
